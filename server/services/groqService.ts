...
interface RoyaltyRule {
  ruleType: 'percentage' | 'tiered' | 'container_size_tiered' | 'minimum_guarantee' | 'cap' | 'deduction' | 'fixed_fee' | 
  ...
  conditions: {
    ...
    territories?: string[];
    containerSizes?: string[]; // Container/package size names for filtering
  ...
  calculation: {
    ...
    containerSizeRates?: Array<{
      size: string;
      baseRate: number;
      volumeThreshold?: number;
      discountedRate?: number;
    }>; // For container/size-based pricing with volume discounts
...
export class GroqService {
  ...
  private async getContractTypePrompts(contractTypeCode?: string): Promise<ContractTypePrompts | null> {
    ...
    try {
      ...
      // Get default prompts for this contract type
      const defaultPrompts = DEFAULT_EXTRACTION_PROMPTS[contractTypeCode];
      
      if (contractType) {
        // Check if DB has ANY non-empty prompt fields
        ...
        if (hasDbPrompts) {
          // Merge DB prompts with defaults - DB values take precedence but fallback to defaults for null fields
          const mergedPrompts: ContractTypePrompts = {
          ...
          console.log(`📋 Loaded custom prompts for contract type: ${contractTypeCode} (merged with defaults)`);
          return mergedPrompts;
        }
      }
      
      // Fallback to default prompts from the module
      if (defaultPrompts) {
        console.log(`📋 Using default prompts for contract type: ${contractTypeCode}`);
  ...
  private async extractRulesFromTable(tableText: string, tableName: string, tableIndex: number, customPrompts?: ContractTypePrompts | null): Promise<RoyaltyRule[]> {
...
2. Look at the COLUMN HEADERS of THIS table only
3. If there is NO "Volume Discount Threshold" or "Discounted Rate" column, set volumeThreshold=0 and discountedRate=baseRate

For container size tables (with sizes like 1-gallon, 4-inch pots):
- Use ruleType: "container_size_tiered"
- Extract into containerSizeRates array

For volume-based tables (with ranges like 1-2,500 units):
- Use ruleType: "tiered"  
- Extract into tiers array with min/max/rate

Return JSON:
{
  "rules": [
    {
      "ruleType": "container_size_tiered" or "tiered",
      "ruleName": "${tableName}",
      "description": "extracted from table",
      "conditions": {},
      "calculation": {
        "containerSizeRates": [...] // for container_size_tiered
        // OR
        "tiers": [...] // for volume tiered
      },
      "priority": ${tableIndex + 1},
      "sourceSpan": {"text": "first 100 chars of table"},
      "confidence": 0.85
    }
  ]
}`;
    ...
    try {
      ...
      const rules = this.sanitizeExtractedRules(result.rules || []);
  ...
  async extractDetailedRoyaltyRules(contractText: string, contractTypeCode?: string): Promise<LicenseRuleExtractionResult> {
    console.log(`🚀 Starting enhanced contract analysis with improved JSON handling...`);
    
    // Load contract type-specific prompts if available
    ...
    let perTableRules: RoyaltyRule[] = [];
    
    if (tables.length >= 2) {
      // Multiple tables detected - use per-table extraction to prevent cross-contamination
      console.log(`📊 [PER-TABLE MODE] Extracting ${tables.length} tables separately to prevent cross-contamination`);
      
      for (let i = 0; i < tables.length; i++) {
        const tableRules = await this.extractRulesFromTable(tables[i].text, tables[i].tableName, i, customPrompts);
    ...
    // Also do consolidated extraction for basic info and any rules not in tables
    const comprehensiveResult = await this.extractAllContractDataInOneCall(contractText, customPrompts);
    ...
    if (perTableRules.length > 0) {
      ...
      // Get container_size_tiered rules from per-table extraction (more accurate)
      const perTableContainerRules = perTableRules.filter(r => r.ruleType === 'container_size_tiered' || r.ruleType === 'tiered');
      ...
      // Get container rules from consolidated
      const consolidatedContainerRules = allRules.filter(r => 
        r.ruleType === 'container_size_tiered' || r.ruleType === 'tiered'
      );
      
      // Get non-container rules from consolidated extraction (fixed fees, minimums, etc.)
      const consolidatedNonContainerRules = allRules.filter(r => 
        r.ruleType !== 'container_size_tiered' && r.ruleType !== 'tiered'
      );
      
      // DEDUPLICATE: Keep best rule per tier (prefer container_size_tiered with valid rates)
      const allPricingRules = [...perTableContainerRules, ...consolidatedContainerRules];
      const tierMap = new Map<string, RoyaltyRule>();
      
      for (const rule of allPricingRules) {
        const tierKey = extractTierNumber(rule.ruleName || '');
        const existing = tierMap.get(tierKey);
        
        // Score rules: prefer container_size_tiered with containerSizeRates
        const hasContainerRates = rule.ruleType === 'container_size_tiered' && 
          (rule.calculation as any)?.containerSizeRates?.length > 0;
        const existingHasRates = existing?.ruleType === 'container_size_tiered' && 
          (existing?.calculation as any)?.containerSizeRates?.length > 0;
        
        if (!existing) {
          tierMap.set(tierKey, rule);
        } else if (hasContainerRates && !existingHasRates) {
          // New rule has container rates, existing doesn't - prefer new
          tierMap.set(tierKey, rule);
        } else if (hasContainerRates && existingHasRates) {
          // Both have container rates - prefer higher confidence
      ...
      allRules = [...deduplicatedPricingRules, ...consolidatedNonContainerRules];
      
      console.log(`🔀 [MERGE] Deduplicated: ${allPricingRules.length} → ${deduplicatedPricingRules.length} pricing rules + ${consolidatedNonContainerRules.length} other = ${allRules.length} total`);
    }
  ...
  private async extractLargeContractInChunks(contractText: string, customPrompts?: ContractTypePrompts | null): Promise<{
    basicInfo: any;
    ...
    const headerRules = await this.extractRulesOnly(contractText.substring(0, 10000), 'licensing', customPrompts);
    ...
    const midRules = await this.extractRulesOnly(midChunk, 'licensing', customPrompts);
    ...
    const tailRules = await this.extractRulesOnly(contractText.substring(tailStart), 'licensing', customPrompts);
...
- "container_size_tiered" - Pricing by container/package size with optional volume discounts
- "percentage" - Percentage-based rates
- "minimum_guarantee" - Minimum payment guarantees
- "fixed_fee" - One-time fixed fees (mother stock, training, setup, materials)
...
**CRITICAL - CONTAINER/SIZE-BASED PRICING TABLES - READ THIS CAREFULLY**:
When you see pricing tables with MULTIPLE ROWS for different sizes/categories (like "1-gallon", "3-gallon", "5-gallon"), 
each row having its OWN rate and volume discount:
- **USE ruleType "container_size_tiered"** - NOT "tiered"!
- Extract ALL rows into calculation.containerSizeRates array
- Each row format: {"size": "1-gallon", "baseRate": 1.25, "volumeThreshold": 5000, "discountedRate": 1.10}

**HOW TO DETECT container_size_tiered (vs regular tiered) - READ CAREFULLY**:
- If table first column has PHYSICAL SIZES (1-gallon, 3-gallon, 4-inch pots, 6-inch pots) → container_size_tiered
- If table first column has QUANTITY RANGES (1-2,500 units, 2,501-7,500 units, 0-1000, 1001-5000) → tiered (NOT container_size_tiered!)
- Column header "Plant Size Category" or "Container Size" → container_size_tiered
- Column header "Sales Volume (Annual)" or "Quantity Range" → tiered (volume-based)

**CRITICAL - "Sales Volume" tables are NOT container size tables!**
A table with "Sales Volume (Annual)" column showing ranges like "1 - 2,500 units" is a VOLUME TIERED table.
Extract as ruleType: "tiered" with tiers array [{min: 1, max: 2500, rate: X}, {min: 2501, max: 7500, rate: Y}]
DO NOT extract as "container_size_tiered" - quantity ranges are NOT container sizes!

**EXAMPLE - Plant Royalty Table (EXTRACT ALL 4 ROWS!)**:
...
EXTRACT ALL 4 ROWS as "container_size_tiered" with containerSizeRates:
...
**IMPORTANT - READ ALL TABLE ROWS**: 
- Count the rows in the table - if there are 4 sizes, extract 4 containerSizeRates entries
- baseRate = the "Royalty per Unit" or "Rate" column value
- discountedRate = the "Discounted Rate" column value (DIFFERENT from baseRate!)
- volumeThreshold = the number from "Volume Discount Threshold" (e.g., "5,000+" → 5000)

**CRITICAL - DO NOT INVENT DISCOUNTED RATES - READ TABLE COLUMN HEADERS CAREFULLY**:
- FIRST: Look at the COLUMN HEADERS of each table separately
- If a table has columns like "Volume Discount Threshold" and "Discounted Rate" → extract those values
- If a table has columns like "Premium Variety Multiplier" and "Seasonal Adjustment" → NO volume discounts exist!
- DO NOT copy volumeThreshold/discountedRate from one table to another table
- Each table is INDEPENDENT - extract only what's in THAT table's columns

**EXAMPLE - Table WITH Volume Discounts (extract volumeThreshold and discountedRate)**:
| Container Size | Royalty Rate | Volume Discount Threshold | Discounted Rate |
| 1-gallon      | $1.25        | 5,000+ units             | $1.10           |
→ Extract: volumeThreshold=5000, discountedRate=1.10

**EXAMPLE - Table WITHOUT Volume Discounts (set volumeThreshold=0, discountedRate=baseRate)**:
| Container Size | Base Royalty Rate | Premium Multiplier | Seasonal Adjustment |
| 4-inch pots   | $0.75            | 1.0x              | +15% spring         |
→ Extract: volumeThreshold=0, discountedRate=0.75 (SAME as baseRate - no discount column!)
→ DO NOT invent volumeThreshold=5000 or discountedRate=0.65 - those don't exist in this table!

- If table does NOT have "Discounted Rate" column → set discountedRate = baseRate (same value)
- If table does NOT have "Volume Discount Threshold" column → set volumeThreshold = 0
- DO NOT calculate, guess, or copy values from other tables

**DO NOT** convert this into quantity-based tiers (min/max/rate format) - that loses the size dimension!

**WRONG** (do NOT do this):
ruleType: "tiered" with tiers: [{min: 0, max: 5000, rate: 1.25}, {min: 5001, rate: 1.10}]
This is WRONG because it loses the per-size rates ($1.25 for 1-gallon, $2.85 for 3-gallon, etc.)

**CORRECT**:
ruleType: "container_size_tiered" with containerSizeRates: [
  {size: "1-gallon", baseRate: 1.25, volumeThreshold: 5000, discountedRate: 1.10},
  {size: "3-gallon", baseRate: 2.85, volumeThreshold: 2000, discountedRate: 2.50}
]
...
Return ONLY valid JSON. No explanations.`;
  ...
  private sanitizeExtractedRules(rules: any[]): RoyaltyRule[] {
    let sanitized = rules
      ...
      .map((r: any) => {
        ...
        // AUTO-DETECT: Convert tiered rules with container sizes to container_size_tiered
        r = this.autoDetectContainerSizePricing(r);
  ...
  private detectAndFixHallucinatedThresholds(rules: any[]): any[] {
    const containerRules = rules.filter(r => r.ruleType === 'container_size_tiered' && r.calculation?.containerSizeRates?.length > 0);
    
    if (containerRules.length < 2) return rules; // Need at least 2 rules to compare
    
    console.log(`🔍 [HALLUCINATION-DETECT] Checking ${containerRules.length} container_size_tiered rules for copied thresholds`);
    
    // Build threshold signature for each rule
    const ruleSignatures = containerRules.map(rule => {
      const rates = rule.calculation.containerSizeRates;
      const thresholds = rates.map((r: any) => r.volumeThreshold || 0).sort((a: number, b: number) => b - a);
      return {
        rule,
        ruleName: rule.ruleName,
        thresholds,
        signature: thresholds.join(','),
        hasNonZeroThresholds: thresholds.some((t: number) => t > 0)
      };
    });
    
    // Log signatures
    ruleSignatures.forEach(sig => {
      console.log(`   [SIGNATURE] "${sig.ruleName}": thresholds=[${sig.signature}], hasNonZero=${sig.hasNonZeroThresholds}`);
    });
    
    // Find duplicate threshold patterns (excluding all-zero patterns)
    const nonZeroSigs = ruleSignatures.filter(s => s.hasNonZeroThresholds);
    const signatureCounts: Record<string, typeof ruleSignatures> = {};
    
    nonZeroSigs.forEach(sig => {
      if (!signatureCounts[sig.signature]) signatureCounts[sig.signature] = [];
      signatureCounts[sig.signature].push(sig);
    });
    
    // Check for duplicates
    for (const [signature, duplicates] of Object.entries(signatureCounts)) {
      if (duplicates.length > 1) {
        console.log(`⚠️ [HALLUCINATION-DETECT] Found ${duplicates.length} rules with IDENTICAL threshold pattern [${signature}]`);
        console.log(`   Rules: ${duplicates.map(d => d.ruleName).join(', ')}`);
        
        // The first rule in document order is likely the real one (Tier 1 usually has discounts)
        // Reset thresholds in subsequent rules with same pattern
        for (let i = 1; i < duplicates.length; i++) {
          const ruleToFix = duplicates[i].rule;
          console.log(`   🔧 [FIX] Resetting hallucinated thresholds in "${duplicates[i].ruleName}"`);
          
          ruleToFix.calculation.containerSizeRates = ruleToFix.calculation.containerSizeRates.map((rate: any) => ({
            ...rate,
            volumeThreshold: 0,
            discountedRate: rate.baseRate // Set to baseRate (no discount)
          }));
        }
      }
    }
  ...
  // Auto-detect and convert container size pricing that was incorrectly classified as tiered
  // SMART CONVERSION: When AI extracts container sizes but puts rates in tier format, we convert
  private autoDetectContainerSizePricing(rule: any): any {
    // Skip if already container_size_tiered or if not a tiered rule
    if (rule.ruleType === 'container_size_tiered') return rule;
    if (rule.ruleType !== 'tiered') return rule;
    
    // Check multiple possible locations for container sizes
    const containerSizes = rule.conditions?.containerSizes || 
                           rule.containerSizes || 
                           [];
    const tiers = rule.calculation?.tiers || rule.volumeTiers || [];
    
    console.log(`🔍 [AUTO-DETECT] Checking rule "${rule.ruleName}": containerSizes=${JSON.stringify(containerSizes)}, tiers count=${tiers.length}`);
    
    // Check if container sizes look like physical sizes (gallon, inch, pot, etc.)
    const sizePatterns = /gallon|inch|pot|container|quart|pint|liter|oz|lb|kg|size|pack/i;
    const hasSizeTerms = containerSizes.length >= 2 && containerSizes.some((s: string) => sizePatterns.test(s));
    
    if (!hasSizeTerms) {
      console.log(`⚠️ [AUTO-DETECT] Skipping - no valid container size terms found`);
      return rule;
    }
    
    // KEY INSIGHT: If AI extracted container sizes, the tier rates are likely per-size rates!
    // The AI often puts per-size rates into volume tier format by mistake.
    // We should convert if we have container sizes, even if tiers have min/max.
    
    // Extract the unique rates from tiers
    const tierRates = tiers.map((t: any) => t.rate).filter((r: any) => r !== undefined);
    
    console.log(`🔄 [AUTO-CONVERT] Converting "${rule.ruleName}" - container sizes detected with ${tiers.length} tiers`);
    console.log(`   Container sizes: ${JSON.stringify(containerSizes)}`);
    console.log(`   Tier rates: ${JSON.stringify(tierRates)}`);
    
    // Build containerSizeRates by mapping container sizes to tier rates
    // Use the tier at index, or fallback to last available tier
    const containerSizeRates = containerSizes.map((size: string, index: number) => {
      const tier = tiers[Math.min(index, tiers.length - 1)] || tiers[0] || {};
      
      // Extract base rate
      const baseRate = tier.rate || tier.baseRate || 0;
      
      // ROBUST VALIDATION: Check if volumeThreshold was EXPLICITLY provided in the tier
      // If tier has no volumeThreshold or max field, the AI is probably hallucinating discounts
      const originalVolumeThreshold = tier.volumeThreshold || tier.max || 0;
      
      // ANTI-HALLUCINATION: Only trust discountedRate if:
      // 1. volumeThreshold was explicitly set (> 0)
      // 2. discountedRate exists and is different from baseRate
      // 3. discountedRate is actually LESS than baseRate (a real discount)
      const hasExplicitThreshold = originalVolumeThreshold > 0;
      const discountedRateExists = tier.discountedRate !== undefined && tier.discountedRate !== null;
      const discountedRateIsLower = discountedRateExists && tier.discountedRate < baseRate;
      
      // Only consider it a real discount if BOTH threshold and discounted rate are present
      const hasRealDiscount = hasExplicitThreshold && discountedRateExists && discountedRateIsLower;
      
      // If no explicit threshold, force no discount (volumeThreshold = 0, discountedRate = baseRate)
      const volumeThreshold = hasRealDiscount ? originalVolumeThreshold : 0;
      const discountedRate = hasRealDiscount ? tier.discountedRate : baseRate;
      
      console.log(`   [SIZE-RATE] ${size}: base=${baseRate}, origThreshold=${originalVolumeThreshold}, discountExists=${discountedRateExists}, hasRealDiscount=${hasRealDiscount} → vol=${volumeThreshold}, disc=${discountedRate}`);
    ...
    console.log(`   Converted rates: ${JSON.stringify(containerSizeRates)}`);
    
    rule.ruleType = 'container_size_tiered';
    rule.calculation = {
      ...rule.calculation,
  ...
  private async extractAllContractDataInOneCall(contractText: string, customPrompts?: ContractTypePrompts | null): Promise<{
    basicInfo: any;
    allRules: RoyaltyRule[];
  }> {
    ...
    if (contractText.length > 15000) {
      ...
      return await this.extractLargeContractInChunks(contractText, customPrompts);
    }
...
${customRuleSection}${sampleOutputSection}
...
Return ONLY valid JSON. No explanations.`;
    ...
    try {
      ...
      const sanitizedRules = this.sanitizeExtractedRules(rawRules);
  ...
  private async extractRulesOnly(contractText: string, documentType: string, customPrompts?: ContractTypePrompts | null): Promise<RoyaltyRule[]> {
    // Build custom rule extraction section if contract type-specific prompt is available
    const customRuleSection = customPrompts?.ruleExtractionPrompt 
      ? `\n**CONTRACT TYPE-SPECIFIC EXTRACTION INSTRUCTIONS:**\n${customPrompts.ruleExtractionPrompt}\n\n`
      : '';
